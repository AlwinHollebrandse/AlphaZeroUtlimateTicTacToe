...
Board data:
  1=white(O), -1=black(X), 0=empty, 2=unavailable
...
class GlobalBoard():
    ...
    def __init__(self, n=3, numberOfLocalBoards=9):
        ...
        self.globalBoard = np.zeros((numberOfLocalBoards, n, n)) # TODO use 0's or blanks for empties
        # self.globalBoard = np.full((numberOfLocalBoards, n, n), 0)
    ...
    def has_legal_moves(self):
        for localBoardIndex in self.validLocalBoardIndex: 
            for row in range(self.n): # TODO correct ordering?
                for col in range(self.n):
                    if self.globalBoard[localBoardIndex][row][col] == 0:
                        return True
        return False
